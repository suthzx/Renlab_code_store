...
def bandplot(kpath, bands, efermi, kpt_bounds, opts, whts=None):
    ...
    for bd in kpt_bounds:
        ax.axvline(x=bd, ls='-', color='k', alpha=0.8) #lw=0.5 # 竖线的宽窄，透明度
    ...
    ax.yaxis.set_minor_locator(AutoMinorLocator(MinorLocator)) # 刻度线密度
    xmin = kpath.min()
    xmax = kpath.max()
    ...
    if opts.kpts:   # KPOINTS-string
        ...
        if os.path.isfile(opts.kpoints) and kp[2][0].upper() == 'L':
            ...
            if opts.section: # 若分段则分段标注K_label
                # ax.set_xticklabels([])
...
if __name__ == '__main__':
    ...
    if os.path.isfile('KPOINTS') and kp[2][0].upper() != 'L':
        ...
        if os.path.isfile("IBZKPT") and os.path.isfile("KPOINTS.band"):
            ...
            # kpath = kpath[skip_kpts:]  - kpath[skip_kpts]  # 去除前IBZKBT的部分 old_mbj
            kpath = kpath[:-skip_kpts]  # 去除最后的部分 new_mbj
            # 减去中间间隔的段长
            forwards = []
            for k in range( int(len(kpath) / per_line))[1:] : #n段，减去n-1段需要前移
                diff = kpath[k*per_line] - kpath[k*per_line-1]
                # print(diff)
                forwards.append(diff)
                kpath[k*per_line:] = kpath[k*per_line:] - diff

            bands = bands[:, :-skip_kpts, :] # 提取Linemode能带信息
            # get boundaries of band path
            xx = np.diff(kpath)
            kpt_bounds = np.concatenate(([0.0,], kpath[1:][np.isclose(xx, 0.0)], [kpath[-1],]))

            # kpath = np.arange(0, nkpts, 1)  # 等间距
            # get boundaries of band path
            ...
            if opts.section:
                ...
                nspin, nkpts, nbands = bands.shape  # 获取原来的shape
                ...
                nkpts = len(kpath)
                bands = np.array(bands_new, dtype=float).reshape((nspin, nkpts, nbands)) # 新的KPATH和BANDS
